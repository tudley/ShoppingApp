...
/*const updateCart = (method, cartItems, cartItem, stock) => {
    ...
    if (method === "adding") {    
        let inStock;
        if (type == null){ // determines if item is in stock, regardless of object structure
            inStock =  stock.filter((element) => (element.id === cartItem.id) && (element.stock > 0))
        } else {
            inStock =  stock.filter((element) => (element.id === cartItem.id) && (element.size[cartItem.size] > 0))
        }
        if (inStock.length > 0) { // if inStock, ammend stock and cart
        ...
        if (type == null) {
            ...
            itemIsInCart = cartItems.filter((item) => (item.id === cartItem.id && item.size === cartItem.size))
        }
